...
| Mixed SOCKS5 / HTTP CONNECT | Implemented for SOCKS5 CONNECT and HTTP CONNECT, with optional shared credentials | GOST, sing-box, xray, local client proxy protocols |

## Research Notes

| Project | Documented support | wrongsv coverage | Next practical gaps |
| --- | --- | --- | --- |
| GOST | Separates proxy protocols from channel protocols. Proxy protocols include HTTP, HTTP/2, SOCKS4/4A, SOCKS5, Shadowsocks, Shadowsocks UDP relay, SNI, and relay. Channels include raw TCP/UDP, TLS/DTLS, WebSocket/WSS, HTTP/2/H2C, gRPC, KCP, QUIC, HTTP/3, and WebTransport. Source: https://latest.gost.run/en/tutorials/protocols/overview/ | Shadowsocks AEAD TCP/UDP plus mixed SOCKS5/HTTP CONNECT cover common GOST proxy protocol families. | SOCKS4/4A, WebSocket/TLS carrier modes. |
...
| Outline | Outline access keys are Shadowsocks-based and support optional TCP/UDP prefixes at the start of the salt for disguises. Source: https://developer.getoutline.org/vpn/advanced/prefixing/ | Standard Shadowsocks AEAD TCP/UDP works without prefixes. | Optional configured server response prefixes. |
...
1. Outline prefix response support and Shadowsocks AEAD-2022.
2. SOCKS4/4A and richer HTTP proxy modes if needed beyond CONNECT.
3. Trojan over TLS.
