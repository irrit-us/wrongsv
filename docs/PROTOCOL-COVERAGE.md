...
| mihomo | VLESS supports REALITY, Vision flow, TCP transport, WebSocket/gRPC/XHTTP-style transports, and UDP packet encodings where empty means raw and `packetaddr`/`xudp` select extended encodings; Shadowsocks supports AEAD and 2022 methods; Trojan proxy entries support TLS, password auth, SNI, certificate verification controls, and UDP. Sources: https://wiki.metacubex.one/en/config/proxies/vless/ and https://wiki.metacubex.one/en/config/proxies/ss/ and https://wiki.metacubex.one/en/config/proxies/trojan/ | VLESS + REALITY + Vision lifecycle tests exist; VLESS + WebSocket TCP and Mux.Cool/XUDP UDP lifecycle tests exist; Shadowsocks AEAD/AEAD-2022 TCP/UDP real-client lifecycle tests added; Trojan TCP/UDP real-client lifecycle tests added. | packetaddr UDP encoding, gRPC/XHTTP carriers. |
...
1. ~~VLESS WebSocket TCP/raw UDP/Mux.Cool XUDP~~ → **Done**. gRPC/XHTTP carriers and packetaddr UDP encoding still open.
