...
| Naive proxy | Implemented for HTTP/2 CONNECT over TLS with HTTP Basic auth, padded request/response framing, configurable padding header, generated or custom TLS certificates, and wrongcl local-proxy coverage. | Naive/naiveproxy |
| Snell v1 TCP CONNECT | Implemented for Snell v1 AEAD stream framing, Argon2id-derived ChaCha20-Poly1305 keys, encrypted CONNECT headers, server tunnel responses, TCP relay, and wrongcl local-proxy coverage. UDP and Snell v2/v3 are not part of this slice. | Surge, mihomo |
