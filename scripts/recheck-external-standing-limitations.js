...
function printHelp() {
  console.log(`recheck-external-standing-limitations.js

Usage:
  node scripts/recheck-external-standing-limitations.js [options] [-- <recheck-standing-limitations args>]
...
function main() {
  ...
  const externalScript = path.join(
    opts.externalRoot,
    "scripts",
    "recheck-standing-limitations.js"
  );
  ...
  if (!fs.existsSync(externalScript)) {
        ...
        JSON.stringify(
          {
            ...
            delegatedCommand: `node scripts/recheck-standing-limitations.js ${opts.passthrough.join(" ")}`.trim(),
          },
          null,
          2
        )
    ...
    emitWrapperError("external_review_helper_missing", {
      ...
      message:
        "recheck-standing-limitations.js not found; pass --external-tests-root or check out wrongsv-external-tests next to wrongsv",
    });
    process.exit(1);
  }
