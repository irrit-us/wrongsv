//! QUIC transport: QUIC connection + stream + VLESS.
//!
...
pub fn connect_quic(
    ...
    hs_rx
        .recv()
        .map_err(|_| io::Error::other("QUIC thread panicked"))??;
...
async fn quic_handshake(
    ...
    let (mut send, mut recv) = connection
        .open_bi()
        .await
        .map_err(|e| io::Error::other(format!("open stream: {e}")))?;
