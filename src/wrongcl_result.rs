...
fn stack_summary_from_document(document: &WrongclClientConfigDocument) -> String {
    ...
    if matches!(
        ...
        return "Naive → h2 CONNECT → TLS → TCP".to_string();
    }
    if matches!(
    ...
    parts.push(match &document.server.endpoint.transport {
        ...
        WrongclTransportDocument::Gdocsviewer { .. } => "Google Docs Viewer",
        ...
        WrongclTransportDocument::Webtransport { .. } => "WebTransport",
    });
